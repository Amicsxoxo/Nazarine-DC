...
# --- NEW: OFF-PEAK VALLEY DETECTION (BEST TIME) ---
...
async def get_best_time():
    ...
    # Find the 3-hour window with the lowest average energy usage
    best_window = None
    ...
    for i in range(len(rows) - 2):
        ...
        if window_avg < lowest_avg:
            ...
            # Grab the start and end hour for the window (e.g., "14:00 - 17:00")
...
ai_forecaster = None  # Global variable to hold our trained model in memory
...
def train_predictive_model():
    ...
    # 1. Extract Historical Data into Pandas
    query = '''
        SELECT timestamp, power_kw FROM energy_history
    '''
    ...
    # 2. Feature Engineering (Teaching the AI about Time and Seasons)
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    df['hour'] = df['timestamp'].dt.hour
    df['dayofweek'] = df['timestamp'].dt.dayofweek
    df['month'] = df['timestamp'].dt.month

    # Group by hour to smooth out the 5-minute sensor noise
    ...
    # It builds 50 decision trees to map out household usage patterns
    ai_forecaster = RandomForestRegressor(n_estimators=50, random_state=42)
    ai_forecaster.fit(X, y)

    return {"status": "success", "message": "Nazarine ML Engine successfully trained on historical telemetry."}

@app.get("/api/forecast")
def get_24h_forecast():
    global ai_forecaster
    
    # Auto-train the model if it hasn't been booted up yet
    if ai_forecaster is None:
        train_predictive_model()

    # 4. Predict the next 24 hours of grid load
    predictions = []
    now = datetime.now()
    ...
    for i in range(24):
        ...
        # Create the exact same features for tomorrow
        ...
        # Ask the AI to predict the kW draw for this specific future hour
